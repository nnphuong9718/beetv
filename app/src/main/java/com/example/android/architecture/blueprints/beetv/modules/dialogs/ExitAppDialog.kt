...
class ExitAppDialog : DialogFragment() {
    ...
    var onClickExitListener : (() -> Unit) ?=null
    ...
    private fun changeBackgroundButton(oldView: View?, newView: View?) {
        ...
        if(newView != null){
            if (newView is MetroItemFrameLayout){
                newView.setBackgroundColor(ContextCompat.getColor(requireContext(),R.color.button_text_unfocus))
